...
static int reg_s5pv210_pd_probe(struct platform_device *pdev)
{
...
/* jason add */
	struct regulator_config pdconfig = {};
	pdconfig.dev = &pdev->dev;
	pdconfig.init_data = config->init_data;
	pdconfig.driver_data = drvdata;
/* end of jason add */
	drvdata->dev = regulator_register(&drvdata->desc, &pdconfig); //jason add
...
static int reg_s5pv210_pd_remove(struct platform_device *pdev)
{
...
module_init(regulator_s5pv210_pd_init);
